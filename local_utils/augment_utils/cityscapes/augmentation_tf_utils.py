...
def resize(img, grt=None, mode='train', align_corners=True):
    ...
    if CFG.AUG.RESIZE_METHOD == 'unpadding':
        ...
        img = tf.compat.v1.image.resize_bilinear(images=img, size=target_size, align_corners=align_corners)
        if grt is not None:
            grt = tf.compat.v1.image.resize_nearest_neighbor(images=grt, size=target_size, align_corners=align_corners)
        ...
        img = tf.compat.v1.image.resize_bilinear(images=img, size=target_size, align_corners=align_corners)
        if grt is not None:
            grt = tf.compat.v1.image.resize_nearest_neighbor(
                images=grt,
                size=target_size,
...
def randomly_scale_image_and_label(image, label=None, scale=1.0, align_corners=True):
    ...
    image = tf.compat.v1.image.resize_bilinear(image, new_dim, align_corners=True)
    if label is not None:
        label = tf.compat.v1.image.resize_nearest_neighbor(label, new_dim, align_corners=True)
    return image, label
...
def preprocess_image_for_train(src_image, label_image):
    ...
    src_image = tf.compat.v1.image.resize_bilinear(
        images=tf.expand_dims(src_image, axis=0),
        size=resize_image_size,
        align_corners=True
    )
    label_image = tf.compat.v1.image.resize_nearest_neighbor(
