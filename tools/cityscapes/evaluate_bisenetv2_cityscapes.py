...
class MultiScaleEvaluator(object):
    ...
    def _scale_crop_evaluate(self, src_input_image, src_label_image): # 여기서 평가
        src_input_image = src_input_image[:, :, :3]
        ...
        '''
        for py in range(0, 1024):  # BGR
            for px in range(0, 1024):
                if src_cloud_image[py][px][0] == 1: # src_cloud_image (1024, 1024, 3) numpy.ndarray
                    preds[py][px] = 0
        '''
